...
public class RemoveSalaryCommand extends SubCommand {
    ...
    public void execute(JavaPlugin javaPlugin, CommandSender sender, String[] args) {
        ...
        if (args.length < 3) {
            Common.sendMessage(sender, "&c/mobcoins removesalary (player) (amount)");
            return;
        }
        ...
        Common.sendMessage(sender, ConfigValue.MESSAGES_SALARY_REMOVED
                .replace("{prefix}", ConfigValue.PREFIX)
